// visuals.js — particles only, background handled by Pixi
...
const PALETTES = {
  ...
  dread:     [[80,20,120],  [60,10,90],   [100,30,140]],
  epic:      [[200,160,20], [240,200,40], [180,140,10]],
  grief:     [[60,80,120],  [80,100,150], [40,60,100]],
  euphoria:  [[220,80,180], [255,120,200],[180,60,160]]
};
...
new p5(function(p) {
  ...
  function spawnParticles(emotion) {
    particles = [];
    const count = emotion === 'danger' ? 25 : emotion === 'tension' ? 15 : 30;
  ...
  p.draw = function () {
    ...
    // FULLY TRANSPARENT — Pixi handles the background
    p.clear();
    ...
    // Particles only — no background fill
    particles.forEach(pt => {
      ...
      switch (currentEmotion) {
        ...
        case 'danger':
        case 'dread':
        case 'epic':             drawSpark(pt);   break;
        default:
          p.noStroke();
          p.fill(pt.r, pt.g, pt.b, pt.alpha * 0.5);
  ...
  function drawFirefly(pt) {
    p.noStroke();
    p.fill(pt.r, pt.g, pt.b, pt.alpha * .35);
  ...
  function drawSpark(pt) {
    p.stroke(pt.r, pt.g, pt.b, pt.alpha * .7);
    p.strokeWeight(1.5);
    const l = pt.size * 2;
    p.line(-l, 0, l, 0);
    p.line(0, -l, 0, l);
